...
use Phalcon\Builder\ModelModule as ModelModuleBuilder;
...
class Scaffold extends Component
{
    ...
    public function build()
    {
        ...
        $modelPath = $config->application->modelsDir[$options['module']].'/'.$modelName.'.php';
        if (!file_exists($modelPath)) {

            $modelBuilder = new ModelModuleBuilder(array(
                'name' => $name,
        ...
        $modelPath = $config->application->modelsDir['base'].'/'.$modelName.'.php';
        ...
        if (isset($options['templateEngine']) && $options['templateEngine'] == 'volt') {
            ...
            //View view.phtml
            $this->_makeViewDetail($path, $options);
    ...
    private function _makeController($path, $options)
    {
        ...
        $path = $options['templatePath'] . '/scaffold/'.$options['form'].'/Controller.php';
        ...
        if (isset($options['modelsNamespace']) === true) {
        	$code = str_replace('$useModel$', 'use '.$options['modelsNamespace'] . "\\" . $options['className'] . ';', $code);
        } else {
            $code = str_replace('$useModel$', ' ', $code);
        }
        
        if (isset($options['gridsNamespace']) === true) {
        	$code = str_replace('$useGrid$', 'use '.$options['gridsNamespace'] . "\\" . $options['className'] . 'Grid;', $code);
        } else {
            $code = str_replace('$useGrid$', ' ', $code);
        }
        
        if (isset($options['formsNamespace']) === true) {
        	$code = str_replace('$useForm$', 'use '.$options['formsNamespace'] . "\\" . $options['className'] . 'Form;', $code);
        } else {
            $code = str_replace('$useForm$', ' ', $code);
        }
        
        if (isset($options['gridsNamespace']) === true) {
        	$code = str_replace('$useDetailview$', 'use '. str_replace('Grids', 'DetailViews', $options['gridsNamespace']) . "\\" . $options['className'] . 'DetailView;', $code);
        } else {
            $code = str_replace('$useDetailview$', ' ', $code);
        }
    ...
    private function makeView($path, $options, $type)
    {
    	...
    	$viewFolder = $options['fileName'];
    	$viewFolder = str_replace('_', '-', lcfirst(Text::uncamelize($viewFolder)));
    	
    	$dirPath = $options['viewsDir'] . $viewFolder;
        ...
        $templatePath = $options['templatePath'] . '/scaffold/'.$options['form'].'/views/' .$type. '.phtml';
        ...
        $code = str_replace('$plural$', str_replace('_', '-', $options['plural']), $code);
    ...
    private function makeViewVolt($path, $options, $type)
    {
        ...
        $templatePath = $options['templatePath'] . '/scaffold/'.$options['form'].'/views/' .$type. '.volt';
        if (!file_exists($templatePath)) {
            throw new BuilderException("Template '" . $templatePath . "' does not exist");
        }
     ...
     */
    private function _makeViewDetail($path, $options)
    {
        $this->makeView($path, $options, 'view');
    }
    ...
    {
        $this->makeView($path, $options, 'create');
    }
    ...
    {
        $this->makeView($path, $options, 'update');
    }
    ...
    private function _makeViewSearch($path, $options)
    {
        ...
        $templatePath = $options['templatePath'] . '/scaffold/'.$options['form'].'/views/search.phtml';
        if (!file_exists($templatePath)) {
            throw new BuilderException("Template '" . $templatePath . "' does not exist");
        }
    ...
    private function _makeViewSearchVolt($path, $options)
    {
        ...
        $templatePath = $options['templatePath'] . '/scaffold/'.$options['form'].'/views/search.volt';
        if (!file_exists($templatePath)) {
            throw new BuilderException("Template '" . $templatePath . "' does not exist");
        }
